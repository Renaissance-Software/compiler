        ...
        Node.ID.type_identifier =>
        {
            switch (type_in_analysis.value.type_identifier.value)
            {
                ...
                TypeIdentifier.ID.function =>
                {
                    ...
                    {
                        const return_type = analyze_type_declaration(compiler, allocator, return_type_node, types);
                        function_type.ret_type = return_type;
                        type_in_analysis.value.type_identifier.value.function.return_type = create_type_node(allocator, return_type);
                    }
                    else
                    {
                    ...
                    {
                        const arg_type = analyze_type_declaration(compiler, allocator, arg_type_node.*, types);
                        function_type.arg_types.append(arg_type) catch |err| {
                            panic("Error allocating argument type\n", .{});
                        };
                        arg_type_node.* = create_type_node(allocator, arg_type);
                    }
                ...
                TypeIdentifier.ID.pointer =>
                {
                    ...
                    const appointee_type = analyze_type_declaration(compiler, allocator, appointee_node, types);
                ...
                TypeIdentifier.ID.array =>
                {
                    ...
                    const array_elem_type = analyze_type_declaration(compiler, allocator, array_elem_type_node, types);
...
pub fn analyze_binary_expression(compiler: *Compiler, allocator: *Allocator, current_function: *Node, current_block: *Node, node: *Node, types: *TypeBuffer, functions: *NodeRefBuffer) void
{
    const binary_op = node.value.binary_expr.id;
    node.value.binary_expr.left = explore_expression(compiler, allocator, current_function, current_block, node.value.binary_expr.left, types, functions);
    node.value.binary_expr.right = explore_expression(compiler, allocator, current_function, current_block, node.value.binary_expr.right, types, functions);
...
pub fn explore_field_identifier_expression(compiler: *Compiler, allocator: *Allocator, current_function: *Node, current_block: *Node, node: *Node, types: *TypeBuffer, functions: *NodeRefBuffer) *Node
{
    switch (node.value)
    {
        Node.ID.identifier_expr =>
...
pub fn explore_expression(compiler: *Compiler, allocator: *Allocator, current_function: *Node, current_block: *Node, node: *Node, types: *TypeBuffer, functions: *NodeRefBuffer) *Node
{
    switch (node.value)
    {
        Node.ID.var_decl =>
        ...
        Node.ID.binary_expr =>
        {
            analyze_binary_expression(compiler, allocator, current_function, current_block, node, types, functions);
        },
        Node.ID.identifier_expr =>
        {
            const name = node.value.identifier_expr.name;
            ...
            var new_node = allocator.create(Node) catch |err| {
        ...
        Node.ID.return_expr =>
        {
            ...
            {
                node.value.return_expr.expression = explore_expression(compiler, allocator, current_function, current_block, return_expr, types, functions);
            }
        },
        Node.ID.block_expr =>
        {
            for (node.value.block_expr.statements.items) |*statement|
            {
                ...
                statement.* = explore_expression(compiler, allocator, current_function, new_current_block, statement.*, types, functions);
            }
        },
        Node.ID.loop_expr =>
        {
            node.value.loop_expr.prefix = explore_expression(compiler, allocator, current_function, current_block, node.value.loop_expr.prefix, types, functions);
            node.value.loop_expr.body = explore_expression(compiler, allocator, current_function, current_block, node.value.loop_expr.body, types, functions);
            node.value.loop_expr.postfix = explore_expression(compiler, allocator, current_function, current_block, node.value.loop_expr.postfix, types, functions);
        },
        Node.ID.branch_expr =>
        {
            node.value.branch_expr.condition = explore_expression(compiler, allocator, current_function, current_block, node.value.branch_expr.condition, types, functions);
            node.value.branch_expr.if_block = explore_expression(compiler, allocator, current_function, current_block, node.value.branch_expr.if_block, types, functions);
            if (node.value.branch_expr.else_block) |else_block|
            {
                node.value.branch_expr.else_block = explore_expression(compiler, allocator, current_function, current_block, else_block, types, functions);
            }
        },
        Node.ID.invoke_expr =>
        {
            ...
            {
                arg.* = explore_expression(compiler, allocator, current_function, current_block, arg.*, types, functions);
            }
        },
        Node.ID.unary_expr =>
        {
            // @TODO: check anything more with unary expression id?
            node.value.unary_expr.node_ref = explore_expression(compiler, allocator, current_function, current_block, node.value.unary_expr.node_ref, types, functions);
        },
        Node.ID.break_expr =>
        {
        ...
        Node.ID.array_lit =>
        {
            ...
            {
                array_elem.* = explore_expression(compiler, allocator, current_function, current_block, array_elem.*, types, functions);
            }
        },
        Node.ID.array_subscript_expr =>
        {
            node.value.array_subscript_expr.expression = explore_expression(compiler, allocator, current_function, current_block, node.value.array_subscript_expr.expression, types, functions);
            node.value.array_subscript_expr.index = explore_expression(compiler, allocator, current_function, current_block, node.value.array_subscript_expr.index, types, functions);
        },
        Node.ID.field_access_expr =>
        {
            node.value.field_access_expr.expression = explore_expression(compiler, allocator, current_function, current_block, node.value.field_access_expr.expression, types, functions);
            node.value.field_access_expr.field_expr = explore_field_identifier_expression(compiler, allocator, current_function, current_block, node.value.field_access_expr.field_expr, types, functions);
        },
        else => panic("ni: {}\n", .{node.value}),
    }
...
pub fn analyze(compiler: *Compiler, allocator: *Allocator, parser_result: *ParserResult) void
{
    ...
    {
        const function_type = analyze_type_declaration(compiler, allocator, function_decl.value.function_decl.type, &types);
        function_decl.value.function_decl.type = create_type_node(allocator, function_type);
        for (function_decl.value.function_decl.arguments.items) |arg, i|
        {
            arg.value.var_decl.var_type = create_type_node(allocator, function_type.value.function.arg_types.items[i]);
        }
    }
    ...
    for (parser_result.function_declarations.items) |function_decl|
    {
        ...
        _ = explore_expression(compiler, allocator, function_decl, main_block, main_block, &types, &parser_result.function_declarations);
    }
}
