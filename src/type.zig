...
pub const Integer = struct
{
    ...
    //fn new(bit_count: u16, signedness: Signedness) Type
    //{
        //return (@enumToInt(TypeKind.integer) << TypeKind.position) | (@enumToInt(signedness) << Signedness.position) | bit_count;
    //}
};
...
pub const Function = struct
{
    ...
    {
        return .{ .value = (@as(u64, @enumToInt(Type.ID.function)) << Type.ID.position) | index };
    }

    pub fn append(function_types: *ArrayList(Type.Function), function_type: Type.Function) Type
    {
...
pub fn new_unresolved_type(index: u64) Type
{
    return .{ .value = (@as(u64, @enumToInt(Type.ID.unresolved)) << Type.ID.position) | index };
}
